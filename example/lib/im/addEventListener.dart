...
import 'package:im_sdk_plugin/enum/ImAdvancedMsgListener.dart';
import 'package:im_sdk_plugin/enum/ImConversationListener.dart';
import 'package:im_sdk_plugin/enum/ImFriendshipListener.dart';
import 'package:im_sdk_plugin/enum/ImGroupListener.dart';
import 'package:im_sdk_plugin/enum/ImSignalingListener.dart';
import 'package:im_sdk_plugin/enum/ImSimpleMsgListener.dart';
...
class AddEventListenerState extends State<AddEventListener> {
  ...
  addEventListener() async {
    ...
    groupListener = ImGroupListener(
      onApplicationProcessed: Provider.of<Event>(context, listen: false).onApplicationProcessed,
      onGrantAdministrator: Provider.of<Event>(context, listen: false).onGrantAdministrator,
      onGroupAttributeChanged: Provider.of<Event>(context, listen: false).onGroupAttributeChanged,
      onGroupCreated: Provider.of<Event>(context, listen: false).onGroupCreated,
      onGroupDismissed: Provider.of<Event>(context, listen: false).onGroupDismissed,
      onGroupInfoChanged: Provider.of<Event>(context, listen: false).onGroupInfoChanged,
      onGroupRecycled: Provider.of<Event>(context, listen: false).onGroupRecycled,
      onMemberEnter: Provider.of<Event>(context, listen: false).onMemberEnter,
      onMemberInfoChanged: Provider.of<Event>(context, listen: false).onMemberInfoChanged,
      onMemberInvited: Provider.of<Event>(context, listen: false).onMemberInvited,
      ...
      onQuitFromGroup: Provider.of<Event>(context, listen: false).onQuitFromGroup,
      onReceiveJoinApplication: Provider.of<Event>(context, listen: false).onReceiveJoinApplication,
      onReceiveRESTCustomData: Provider.of<Event>(context, listen: false).onReceiveRESTCustomData,
      onRevokeAdministrator: Provider.of<Event>(context, listen: false).onRevokeAdministrator,
    ...
    advancedMsgListener = new ImAdvancedMsgListener(
      onRecvC2CReadReceipt: Provider.of<Event>(context, listen: false).onRecvC2CReadReceipt,
      onRecvMessageRevoked: Provider.of<Event>(context, listen: false).onRecvMessageRevoked,
      onRecvNewMessage: Provider.of<Event>(context, listen: false).onRecvNewMessage,
      onSendMessageProgress: Provider.of<Event>(context, listen: false).onSendMessageProgress,
      ...
      onRecvMessageExtensionsChanged: (String msgID, List<ImMessageExtension> extList) {
      ...
      onMessageDownloadProgressCallback: (ImMessageDownloadProgress messageProgress) {
        print("下载进度");
    ...
    signalingListener = new ImSignalingListener(
      onInvitationCancelled: Provider.of<Event>(context, listen: false).onInvitationCancelled,
      onInvitationTimeout: Provider.of<Event>(context, listen: false).onInvitationTimeout,
      onInviteeAccepted: Provider.of<Event>(context, listen: false).onInviteeAccepted,
      onInviteeRejected: Provider.of<Event>(context, listen: false).onInviteeRejected,
      onReceiveNewInvitation: Provider.of<Event>(context, listen: false).onReceiveNewInvitation,
    );

    conversationListener = new ImConversationListener(
      onConversationChanged: Provider.of<Event>(context, listen: false).onConversationChanged,
      onNewConversation: Provider.of<Event>(context, listen: false).onNewConversation,
      onSyncServerFailed: Provider.of<Event>(context, listen: false).onSyncServerFailed,
      onSyncServerFinish: Provider.of<Event>(context, listen: false).onSyncServerFinish,
      onSyncServerStart: Provider.of<Event>(context, listen: false).onSyncServerStart,
    ...
    friendshipListener = new ImFriendshipListener(
      ...
      onFriendInfoChanged: Provider.of<Event>(context, listen: false).onFriendInfoChanged,
    );
    //注册简单消息监听器
    // ignore: deprecated_member_use
    await ImSDKPlugin.imManager.addSimpleMsgListener(listener: simpleMsgListener);
    //注册群组消息监听器
    await ImSDKPlugin.imManager.addGroupListener(listener: groupListener);
    //注册高级消息监听器
    await ImSDKPlugin.imManager.getMessageManager().addAdvancedMsgListener(
      listener: advancedMsgListener,
    );

    //注册会话监听器
    await ImSDKPlugin.imManager.getConversationManager().addConversationListener(
      listener: conversationListener,
    );
    //注册关系链监听器
    await ImSDKPlugin.imManager.getFriendshipManager().addFriendListener(
      listener: friendshipListener,
    );
    //注册信令消息监听器
    await ImSDKPlugin.imManager.getSignalingManager().addSignalingListener(
  ...
  removeAdvanceMsgListener() async {
    await ImSDKPlugin.imManager.getMessageManager().removeAdvancedMsgListener(
      listener: advancedMsgListener,
    );
  }
  ...
  removeSignalingListener() async {
    await ImSDKPlugin.imManager.getSignalingManager().removeSignalingListener(
      listener: signalingListener,
    );
  }
  ...
  removeGroupListener() async {
    await ImSDKPlugin.imManager.removeGroupListener();
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      child: Column(
        children: [
          new ElevatedButton(onPressed: addEventListener, child: Text(imt("注册事件"))),
          new ElevatedButton(
          ...
          new ElevatedButton(onPressed: removeGroupListener, child: Text(imt("注销gruopListener"))),
        ],
      ),
    );
  }
