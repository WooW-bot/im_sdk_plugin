...
import 'package:im_sdk_plugin/enum/message_priority_enum.dart';
...
class SendFileMessageState extends State<SendFileMessage> {
  ...
  sendFileMessage() async {
    ...
    ImValueCallback<ImMsgCreateInfoResult> createMessage =
        await ImSDKPlugin.imManager.getMessageManager().createFileMessage(
          filePath: image!.path,
          fileName: fileName,
      ...
      child: Column(
        children: [
          Row(
            children: [
              Container(
                child: ElevatedButton(onPressed: getImage, child: Text(imt("选择文件"))),
          ...
          Row(
            children: [
              Expanded(
                child: ElevatedButton(onPressed: sendFileMessage, child: Text(imt("发送文件消息"))),
              ),
            ],
          ),
          SDKResponse(resData),
