...
class SendFaceMessageState extends State<SendFaceMessage> {
  ...
  sendFaceMessage() async {
    ImValueCallback<ImMsgCreateInfoResult> createMessage =
        await ImSDKPlugin.imManager.getMessageManager().createFaceMessage(index: index, data: data);
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: ElevatedButton(onPressed: sendFaceMessage, child: Text(imt("发送表情消息"))),
              ),
            ],
          ),
          SDKResponse(resData),
