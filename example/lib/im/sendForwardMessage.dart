...
class SendForwardMessageState extends State<SendForwardMessage> {
  ...
  sendForwardMessage() async {
    ImValueCallback<ImMsgCreateInfoResult> createMessage =
        await ImSDKPlugin.imManager.getMessageManager().createForwardMessage(msgID: msgIDs.first);
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: ElevatedButton(onPressed: sendForwardMessage, child: Text(imt("转发消息"))),
              ),
            ],
          ),
          SDKResponse(resData),
