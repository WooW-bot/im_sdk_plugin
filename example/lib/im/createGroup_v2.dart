...
import 'package:im_sdk_plugin/enum/group_add_opt_enum.dart';
...
class CreateGroupV2State extends State<CreateGroupV2> {
  ...
  GroupAddOptTypeEnum? addOpt = GroupAddOptTypeEnum.IM_GROUP_ADD_AUTH;
  ...
  createGroupv2() async {
    ...
    for (String userID in memberList) {
      list.add(
        ImGroupMember(userID: userID, role: GroupMemberRoleTypeEnum.IM_GROUP_MEMBER_ROLE_MEMBER),
      );
    }
    ImValueCallback<String> res = await ImSDKPlugin.imManager.getGroupManager().createGroup(
      groupType: groupType,
      groupName: groupName,
      groupID: groupID,
      ...
      child: Column(
        children: [
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                              ...
                              child: ElevatedButton(
                                onPressed: () {
                                  showAdaptiveActionSheet(
                                    ...
                                    actions: <BottomSheetAction>[
                                      BottomSheetAction(
                                        title: const Text('IM_GROUP_ADD_ANY'),
                                        onPressed: (context) {
                                          setState(() {
                                            addOpt = GroupAddOptTypeEnum.IM_GROUP_ADD_ANY;
                                          });
                                          Navigator.pop(context);
                                        },
                                      ),
                                      BottomSheetAction(
                                        title: const Text('IM_GROUP_ADD_AUTH'),
                                        onPressed: (context) {
                                          setState(() {
                                            addOpt = GroupAddOptTypeEnum.IM_GROUP_ADD_AUTH;
                                          });
                                          Navigator.pop(context);
                                        },
                                      ),
                                      BottomSheetAction(
                                        title: const Text('IM_GROUP_ADD_FORBID'),
                                        onPressed: (context) {
                                          setState(() {
                                            addOpt = GroupAddOptTypeEnum.IM_GROUP_ADD_FORBID;
                                          });
                                          Navigator.pop(context);
                                        },
                                      ),
                            ...
                            Container(margin: EdgeInsets.only(left: 12), child: Text("已选：$addOpt")),
                          ],
                        ),
                      ),
                    ],
          ...
          new Row(
            children: [
              Expanded(
                child: ElevatedButton(onPressed: createGroupv2, child: Text(imt("高级创建群"))),
              ),
            ],
          ),
          SDKResponse(resData),
