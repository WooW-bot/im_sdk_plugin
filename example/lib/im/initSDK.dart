...
import 'package:im_sdk_plugin/enum/ImSDKListener.dart';
import 'package:im_sdk_plugin/enum/log_level_enum.dart';
...
class InitSDKState extends State<InitSDK> {
  ...
  initIMSDK() async {
    ImValueCallback<bool> res = await ImSDKPlugin.imManager.initSDK(
      appID: int.parse(storage.getItem('sdkappid')),
      loglevel: LogLevelEnum.IM_LOG_DEBUG,
      showImLog: true,
      listener: new ImSDKListener(
        onConnectFailed: Provider.of<Event>(
          context,
          listen: false,
        ).onConnectFailed,
        onConnectSuccess: Provider.of<Event>(
          context,
          listen: false,
        ).onConnectSuccess,
        onConnecting: Provider.of<Event>(
          context,
          listen: false,
        ).onConnectSuccess,
        onKickedOffline: Provider.of<Event>(
          context,
          listen: false,
        ).onKickedOffline,
        onSelfInfoUpdated: Provider.of<Event>(
          context,
          listen: false,
        ).onSelfInfoUpdated,
        onUserSigExpired: Provider.of<Event>(
          context,
          listen: false,
        ).onUserSigExpired,
        onUserStatusChanged: Provider.of<Event>(
          context,
          listen: false,
        ).onUserStatusChanged,
